...
class LoanServiceTest {
    ...
    @InjectMocks // 테스트 대상에게 가짜 객채를 주입
    ...
    class CreateLoan {
        ...
        void createLoan_Success() {
            ...
            Book book = new Book(bookId,"클린 코드", "로버트 마틴", "978-023456789", 30000);

            // Mock 동작 정의
            // loanService.createLoan()을 호출하면서 발생할 수 있는 모든 상황을 가정하여 Mock 객체에 세팅
            ...
            // 이 메서드가 실제로 호출되었나? 를 확인
            // 가짜 객체는 실제로 DB 연동하지 않기 떄문에 서비스의 특정 메서드가 호출될 떄
        ...
        void createLoan_MemberNotFound() {
            ...
            assertThatThrownBy(() -> loanService.createLoan(request))
                    ...
                    .hasMessage(String.valueOf(memberId));
        ...
        void createLoan_ExceedLimit() {
            ...
            when(loanRepository.countByMemberIdAndReturnDateIsNull(memberId)).thenReturn(4L);
            when(loanRepository.existsByBookIdAndReturnDateIsNull(bookId)).thenReturn(false);
            when(loanRepository.existsOverdueLoan(memberId)).thenReturn(true);

            LoanCreateRequest request = new LoanCreateRequest(memberId, bookId);

            // when
            assertThatThrownBy(() -> loanService.createLoan(request)).isInstanceOf(LoanLimitExceededException.class)
        ...
        void createLoan_checkDueDate() {
            ...
            // 단순히 호출 되었냐 만 확인 -> any()
            // 전달된 객체의 값 검증 필요 -> ArgumentCaptor 호출
        ...
        4. verify() 사용 시기
        - 중요한 비즈니스 규칙 검증: 대출 한도체크, 연체 여부 확인
        - 부작용이 있는 동작: save, delete, 외부 API 호출
        - 예외 케이스 작성 시에는 never()를 사용해서 호출되지 않았는지를 검증.

        5. 검증 시 메세지까지 검증 권장 (hasMessageContaining을 통해 핵심 키워드만 확인해도 좋아요.)
         */
    }
}