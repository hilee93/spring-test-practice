...
class BookControllerTest {
    ...
    class GetBooks {
        ...
        void getAllBooks() throws Exception {
            ...
            List<BookResponse> books = List.of(
                    ...
                    new BookResponse(2L, "아무책", "김춘식", "456", 35000, null)
            );
        ...
        void getBookById() throws Exception {
            // given
            BookResponse book = new BookResponse(1L, "클린 코드", "로버트 마틴", "123", 30000, null);
    ...
    class CreateBook {
        ...
        void createBook_Success() throws Exception {
            // given
            BookCreateRequest request = new BookCreateRequest("클린 코드", "로버트 마틴", "123-0123456789", 30000, null);
            BookResponse response = new BookResponse(1L, "클린 코드", "로버트 마틴", "123-0123456789", 30000, null);

            when(bookService.createBook(any(BookCreateRequest.class)))
                    .thenReturn(response);
            // when
            mockMvc.perform(post("/api/books") // post 요청 보낼거야
                    .contentType(MediaType.APPLICATION_JSON) // JSON 데이터 보낼거야
                    .content(objectMapper.writeValueAsString(request))) // 자바 객체를 JSON 문자열로 변환해서 보내줘
                    .andExpect(status().isCreated())
                    .andExpect(header().exists("Location")) // header에 Location이라는 정보가 있을 것이다.
